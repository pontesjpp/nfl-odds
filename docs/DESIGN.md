...
\n## EWM Decay Choice\nNote: Exponential Weighted Moving Averages (EWM) are intentionally decayed by *game played*, not by calendar days/weeks. A player returning from a bye week or a long injury will not suffer additional "rust" penalty beyond the normal game decay in these specific features. If future models require rust evaluation, a dedicated feature (e.g., `weeks_since_last_game`) should be added rather than artificially diluting the production EWM.
